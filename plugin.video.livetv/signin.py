...
class SignInDialog(xbmcgui.WindowDialog):
	...
	def __init__(self, *args, **kwargs):
		...
		self.text_signup_url	  = 'http://www.mediaplayerx9.com'
	...
	def setActions(self):
		...
		#self.password_textbox.controlDown(self.autologin_toggle_button)
		...
		self.last_name_textbox.controlDown(self.create_email_textbox)
		self.last_name_textbox.controlLeft(self.first_name_textbox)
		self.last_name_textbox.controlRight(self.first_name_textbox)
		self.create_email_textbox.controlUp(self.last_name_textbox)
		self.create_email_textbox.controlDown(self.create_username_textbox)
		self.create_username_textbox.controlUp(self.create_email_textbox)
		self.create_username_textbox.controlDown(self.create_password_textbox)
		...
		self.submit_signup_button.controlDown(self.first_name_textbox)#self.submit_signup_button.controlDown(self.submit_signuptrial_button)
		self.submit_signup_button.controlLeft(self.cancel_signup_button)
		...
		self.cancel_signup_button.controlUp(self.confirm_password_textbox)
		...
		self.cancel_signup_button.controlRight(self.submit_signup_button)
...
def send2remote(mode="openaddon", data=[]):
	url = gd.BASE_URL+"/iptv-api.php"
	if mode == "openaddon":
		post_data = {}
		post_data["openaddon"] = ''
		response = gd.net.http_POST(url,post_data)
		print "Returned response from server:"
...
class LicenseDialog(xbmcgui.WindowDialog):
	...
	def __init__(self, *args, **kwargs):
		...
		self.image_background = self.image_dir+"/bg_screen1.jpg"
		self.image_header_bg = self.image_dir+"/header-bg1.png"
		self.imgheaderbg = xbmcgui.ControlImage(self.screenx, self.screeny, self.screenx, 50, self.image_header_bg)
		self.imgbackground = xbmcgui.ControlImage(self.screenx, self.screeny, self.screenx, self.screeny, self.image_background)
		self.addControl(self.imgbackground)
		self.addControl(self.imgheaderbg)
		...
		X = int((FORM3_X)+400)
		Y = int((FORM3_Y)+285)
		self.create_username_textbox = xbmcgui.ControlEdit(X, Y, W, H, '', textColor="0xFF777777", focusTexture=self.image_txt_input_selc, noFocusTexture=self.image_txt_input, _alignment=6|4)
		self.create_username_textbox.setPosition(X, Y)
		self.create_username_textbox.setHeight(H)
		self.create_username_textbox.setWidth(W)
		self.addControl(self.create_username_textbox)
