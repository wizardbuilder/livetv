...
def populate(url, category, cat_id=0, use_cache=False, extra=None):
    print '-------------------populate-------------------'
    print 'url:'+url
    ...
    if info["user_info"]['auth'] == 1:
        ...
        for chan in channels:
            ...
            if category_name:
                ...
                if res: # Incase result was 'None'
                    categories = res
        categories = getinfo(gd.CAT_URL+'/'+str(cat_id))
        for value in categories:
            ...
            fanart = ""
            bCat = False
            plugintools.add_item( action=action , title=value['name'] , thumbnail=value['image'] , fanart=value['image'] , isPlayable=False, folder=True )
            # Display categories
        ...
        if bCat:
            if category != 'Uitbreiden?':
                epg_quit = os.path.join(gd.datapath, "quit")
                if not os.path.exists(epg_quit):
                    xbmc.executebuiltin('XBMC.RunScript(script.tvguide)')
                    sys.exit()
                else:
                    print 'epg_quit:'+epg_quit
                    os.remove(epg_quit)
...
def run():
    ...
    if params.get("action") is None:
        ...
        populate(url, category, cat_id, True)
    else:
        pass
    plugintools.close_item_list()
