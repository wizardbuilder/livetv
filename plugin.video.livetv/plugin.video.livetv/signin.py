...
class SignInDialog(xbmcgui.WindowDialog):
	...
	def __init__(self, *args, **kwargs):
		...
		self.image_s_button_wifi  = self.image_dir+"/wifi.png"
	...
	def buildElements(self): 
		...
		if xbmc.getCondVisibility('system.platform.linux'):
			...
			X = int((FORM1_X+200))
			Y = int((FORM1_Y+350))
			self.wifi_button = xbmcgui.ControlButton(X, Y, W, H, label='', textColor="0xFF777777", focusTexture=self.image_s_button_wifi, noFocusTexture=self.image_s_button_wifi, alignment=2|4)
			self.wifi_button.setLabel(xbmcaddon.Addon(id=gd.addon_id).getLocalizedString(7016))
			self.addControl(self.wifi_button)
			self.firstWindow.append(self.wifi_button)
		...
		self.first_name_textbox.setText("Voornaam")
		...
		self.last_name_textbox.setText("Achternaam")
		...
		self.create_email_textbox.setText("Uw Email adres")
		...
		self.create_username_textbox.setText("Kies een gebruikersnaam")
		...
		#self.create_password_textbox.setText('PASSWORD1')
		...
		#self.create_password_textbox.setText("PASSWORD2")
	...
	def setActions(self):
		# winset #1
		if xbmc.getCondVisibility('system.platform.linux'):
			self.username_textbox.controlUp(self.wifi_button)
		else:
		...
		if xbmc.getCondVisibility('system.platform.linux'):
			self.new_acct_button.controlDown(self.wifi_button)
		else:
			self.new_acct_button.controlDown(self.username_textbox)
		if xbmc.getCondVisibility('system.platform.linux'):
			self.wifi_button.controlUp(self.new_acct_button)
			self.wifi_button.controlDown(self.username_textbox)
	...
	def handler_loginresponse(self):
		...
		if " " in username:
			print "handler_loginresponse check"
			xbmcgui.Dialog().ok(xbmcaddon.Addon(id=gd.addon_id).getLocalizedString(7104), '',xbmcaddon.Addon(id=gd.addon_id).getLocalizedString(7103),'')
			return
...
def send2remote(mode="openaddon", data=[]):
	...
	try:
		...
		if mode == "signup":
			...
			print post_data
			response = gd.net.http_POST(url,post_data)
			print "Returned signup response from server:"+response.content
			return json.loads(response.content)
		if mode == "signuptrial":
			post_data = {}
			post_data["signup"]   = ''
			...
			print post_data
			response = gd.net.http_POST(url,post_data)
			print "Returned signuptrial response from server:"+response.content
			return json.loads(response.content)
		if mode == "signin":
			post_data = {}
			post_data["signin"]   = ''
			...
			post_data["password"] = data[0]["password"]
			response = gd.net.http_POST(url,post_data)
			print "Returned signin response from server:"+response.content
			return json.loads(response.content)
	except:
		return {"response": "errors", "errors": []}


def setClickflixCredentials(email, password):
	url = gd.CLICKFLIX_URL
	print ('setClickflixCredentials: configure emby')
	post_data = {}
	post_data["email"] = email
	post_data["password"] = password
	#print "http_POST url"
	#print url
	#print "http_POST post_data"
	#print post_data
	#response = gd.net.http_POST(url,post_data)
	#print "http_POST response"
	#print response.content
	try:
		response = gd.net.http_POST(url,post_data)
		result = json.loads(response.content)
	except:
		print ('setClickflixCredentials: Something went wrong parsing the result from clickflix')
		return False
	try:
		if result and result['success']:
			gd.clickflix_addon.setSetting("username",email)
			gd.clickflix_addon.setSetting("password",password)
			gd.clickflix_addon.setSetting("ipaddress",gd.EMBY_URL)
			gd.clickflix_addon.setSetting("port",gd.EMBY_PORT)
			return True
		return False
	except:
		print ('setClickflixCredentials: Something went wrong configuring the emby settings xml')
		return False
